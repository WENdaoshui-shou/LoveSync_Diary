...
class Profile(models.Model):
    ...
    def accept_couple_request(self):
        ...
        # 创建CoupleRelation记录
        from couple.models import CoupleRelation
        CoupleRelation.objects.create(
            user1=self.user,
            user2=requester.user,
            relationship_start_date=now.date()
        )
    ...
    def break_up(self):
        ...
        try:
            # 尝试两种可能的组合
            relation = CoupleRelation.objects.filter(
                (models.Q(user1=self.user) & models.Q(user2=partner.user)) |
                (models.Q(user1=partner.user) & models.Q(user2=self.user))
            ).first()
            if relation:
                relation.delete()
        except:
...
def create_and_save_user_profile(sender, instance, created, **kwargs):
    if created:
        ...
        # 为新用户创建默认VIP记录
        from vip.models import VIPMember
